...
class DebugWindow(gtk.Dialog):
    ...
    def __init__(self, parent=None):
        gtk.Dialog.__init__(self, 'A ocurrido un error', parent,
                            gtk.DIALOG_MODAL | gtk.DIALOG_NO_SEPARATOR)

        self._build_ui()
        #self.send = self._create_button(gtk.STOCK_NETWORK, _('_Reportar error ...'))
        #self.send.connect('clicked', self._on_send__clicked)
        #self.action_area.pack_end(self.send)

        
        quit = self._create_button(gtk.STOCK_OK, _('_Continuar'))
    ...
    def _build_ui(self):
        ...
        lbl = gtk.Label('<b>Tipo de Excepción:</b>')
