...
class wnMaquinaria (GladeConnect):
        
    def __init__(self, conexion=None, padre=None, root="wnMaquinaria"):
        GladeConnect.__init__(self, "glade/wnMaquinaria.glade", root)
        self.cnx = conexion
        self.cursor = self.cnx.cursor()
        self.padre = padre
        if padre is None :
            self.wnMaquinaria.maximize()
            self.frm_padre = self.wnMaquinaria
        else:
    ...
    def crea_columnas(self):
        ...
        SimpleTree.GenColsByModel(self.modelo, columnas, self.treeMaquinaria)
    ...
    def on_btnAnadir_clicked(self, btn=None, data=None):
        ...
        dlg.editando=False
        response = dlg.dlgMaquinaria.run()
    ...
    def on_btnQuitar_clicked(self, btn=None):
        ...
        if dialogos.yesno("¿Desea eliminar la Maquinaria <b>%s</b>?\nEsta acción no se puede deshacer\n" % descripcion, self.frm_padre) == gtk.RESPONSE_YES:
            llaves={'descripcion_maquinaria':descripcion}
            ...
            try:
                ...
                dialogos.error("<b>NO SE PUEDE ELIMINAR</b>\nLa Maquinaria <b>%s</b>, esta relacionada con un producto"%descripcion)
    ...
    def on_btnCerrar_clicked(self, btn=None):
        if self.padre is None:
            ...
            self.padre.remove_tab("Maquinaria")
            
    def on_treeMaquinaria_row_activated(self, tree=None, path=None, col=None):
        self.on_btnPropiedades_clicked()


class dlgMaquinaria(GladeConnect):
    def __init__(self, conexion=None, padre=None, editando=None):
        GladeConnect.__init__(self, "glade/wnMaquinaria.glade", "dlgMaquinaria")
        self.cnx = conexion
        self.cursor = self.cnx.cursor()
        ...
        self.dlgMaquinaria.show()
    
    def on_btnAceptar_clicked(self, btn=None, date=None, cnx=None):
        if self.entDescripcion.get_text() == "":
            dialogos.error("El campo <b>Descripción Maquinaria</b> no puede estar vacío")
            return
        
        campos = {}
        llaves = {}
        campos['descripcion_maquinaria']  = self.entDescripcion.get_text().upper()
        if not self.editando:
            sql = ifd.insertFromDict(schema + "." + table, campos)
            
        else:
            llaves['codigo_maquinaria'] = self.entCodigo.get_text().upper()
            sql, campos=ifd.updateFromDict(schema + "." + table, campos, llaves)
            
        self.cursor.execute(sql, campos)
        self.dlgMaquinaria.hide()
